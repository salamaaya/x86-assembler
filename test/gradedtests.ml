...
(* Hw Split helpers *)
let as_ungraded_test = function
  | GradedTest (name, _, tests) -> Test(name, tests)
  | test -> test

let as_ungraded_tests = List.map as_ungraded_test

let prepend_test_name_with = fun prefix -> function
  | GradedTest(name, score, tests) ->
      GradedTest(prefix ^ name, score, tests)
  | Test(name, test) ->
      Test(prefix ^ name, test)

let prepend_tests_names_with = fun prefix ->
  List.map (prepend_test_name_with prefix)
...
let part1_tests : suite = prepend_tests_names_with "[Part I] " [
...
let part2_tests: suite = prepend_tests_names_with "[Part II] " [
    ...
    GradedTest ("X86lite Assembly Programming (manual)", 10, [
...
let graded_tests :suite = (as_ungraded_tests part1_tests) @ part2_tests
